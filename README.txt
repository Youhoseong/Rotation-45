...
- trivial 알고리즘은 대략 N*d만큼의 계산을 수행해야 한다. 테스트한 컴퓨터는 1초에 약 1억 7000만번의 계산을 수행한다.
  그래서 N*d/170000000 의 값을 계산하면 trivial의 계산 시간을 대략적으로 추론할 수 있다.

- juggle 알고리즘은 trivial 알고리즘의 계산량에서 Rotate 횟수인 d로 나눈 만큼의 계산을 수행해야 한다. 

- blockswap 알고리즘은 문자열을 d와 N-d 크기의 블록으로 묶고 재귀적으로 함수를 호출해서 swap을 수행한다. 때문에 원소를 하나씩 옮기는 tirivial solution과 juggle 알고리즘보다 훨씬 빠르다. d의 값이 N/2의 값에 가까워 질 수록 더 빨라진다. 예를 들어서 10만 자리의 문자열을 d를 5만으로 한다면 reverse보다 계산시간이 빠르게 된다.
  

- reverse 알고리즘은 처음에 d/2만큼의 문자열 원소를 reverse하고 
